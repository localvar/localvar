...
Let's start with a simple question: we have an integer with initial value `0` as a counter, if we want to write a function which adds `1` to it in each call, but reset it to `0` after its value reaches 10, how should this function be implemented?
...
Okay, then let's make it a little harder by putting it into a concurrent environment. Seems still not difficult, we only need to add a lock:
...
This is correct, but if we count performance and memory usage in, then it is not good enough: for performance, though the implementation of `Lock` differs from operating systems, it usually requires switch from user mode to kernel mode and then back to user mode, compare to the simple self increment operation, this is too expensive; for memory usage, `counter` only needs 4 bytes typically, but `lock` may use as many as dozens of bytes, which is also big waste.

To avoid these two issues, we need to introduce the protagonist of this article, "Lock-free multithreading technologies".

But don't be fooled by the word "lock-free": strictly speaking, as long as there are write operations in the concurrent access of multiple threads to the same object, a lock is needed. The so-called lock-free is just mean "no additional lock is required (i.e., the object to be protected by the lock itself is used as the lock)" and/or "only use light weight locks (never enter the kernel)".
...
But CPU reordering is tricky, for example: suppose `x`, `y` are two integer variables with initial value 0, and thread 1 executes (we should use assembly code here, but using C/C++ code makes it easier to understand, and since all the statements are simple assignment operations, we can assume that each line of code corresponds to one CPU instruction): 
...
Well, the unpredictable execution order of the two threads leads to an unpredictable final result, that's sound correct. So, although the CPU caused reordering is more complex, it looks nothing more than that either.
...
Oops, not only we see that `r1` and `r2` are both 0, but also the probability is very high. And this is the CPU reordering!

I think everyone will get confused when he/she first see these counter-intuitive things, and even not confused any more, it is still not easy to make things 100% correct, because control the code that don't follow the common sense is sometimes very subtle, and mistakes can be made as the result of a most slightly careless.

To understand lock-free multithreading, resolve confusions and avoid mistakes, we need to learn some theoretical knowledge. Let's get started.
...
And the most important atomic operation is the CAS(compare and swap) operation, which we will talk about in details later.

Though CPUs support atomic operations, there are differences on how they support them. Thus, all major programming languages provide a set of equivalent APIs through built-in functions or library to mask these differences and improve portability. For examples:
...
Among the commonly CPUs, the x86/64 series uses the "strong" model, so `SL` may become `LS`, this why we can see that `r1` and `r2` are both 0 in the example above.

And the arm/arm64 series uses the "weak with data dependency" model. Here, "data dependency" means that the latter operation depends on the result of the former operation, for example, coping one byte from address A to address B is a combination of `LS` that reads first and then writes, because the data to be written is the result of the read operation, there's a dependency between these two operations. On CPUs of this model, combinations with similar dependencies are not allowed to be reversed.

Because arm/arm64 series uses a weaker model, some programs that don't have problems on x86/64 will have a problem on arm/arm64. To show the difference, we can revise the previous example to make thread 1 to execute `SS`:
...
However, I didn't see the desired result by simply make the above modifications to the code on arm/arm64, actually, I took a lot of effort to change the code to below(please refer to [《与程序员相关的CPU缓存知识》](https://coolshell.cn/articles/20793.html) to find out why) before I saw the result.
...
At the time I writing this part, Apple released its new M1 MacBook, with a huge edge over x86/64 in terms of performance and power consumption. But as the M1 chip is in the ARM camp, we developers could not be happy yet, because, if portability haven't been considered before, migrate our programs written for x86/64 to it by a simple re-compilation will result in great potential risks.

# Memory Order

Due to the application of technologies such as superpipelined, multiple issue, superscalar, etc., modern CPUs may execute multiple instructions at the same time even there's only one core. However, for our programs, the behavior of the CPU is only visible when there is a memory (including CPU cache) access, so we can think of the order in which the CPU accesses memory is the order in which it executes instructions. Therefore, as long as we can control the order of memory access, we can avoid the issues caused by the CPU memory models.
...
If a read operation uses the `acquire` memory order, then no operation in the current thread is allowed to be reordered before the read, but it does not prevent operations being reordered after the read. For example, suppose `x`, `y`, `z` are three integer atomic variables:
...
Then, writting to `z` will never be executed before reading from `y` because `acquire` don't allow this kind of reordering. But writting to `x` may be executed after reading from `y` because `acquire` allows it.
...
If a write operation uses the `release` memory order, then no operation in the current thread is allowed to be reordered after the write, but it does not prevent operations being reordered before the write. For example, suppose `x`, `y`, `z` are three integer atomic variables again:
...
Then, writting to `x` will never be executed after writing to `y` because `release` don't allow this kind of reordering. But writting to `z` may be executed before writing to `y` because `release` allows it.
...
At the first glance, `sequentially consistent` seems to be the simplest one of the six memory orders, but this is a misunderstand. It seems simple because it seems to mean "reorder is completely prohibited, and the code must be executed in the order in which they are written"; It is misunderstanding because it means more than that, otherwise, the same can be done via using `acquire` for all read, `release` for all write, and `acquire-release` for all read-modify-write, why introduce a new `sequential consistent`? 
...
Take the following code as an example: suppose there are 4 threads, each executes one of the 4 functions, because all operations are `sequentially consistent`, so if one thread sees A happens before C, then no other threads can see C happens before A, and vice versa, this means, when all threads finish executing, `z` can't be 0.

However, even we see A happens before C, and know that B happens before C because of `write_a_then_y`, we still can't tell the order of A and C(although theoretically it must be one after the other), this is why this memory order is named with "consistent" rather than other words.
...
If you're still confused, please think about this: one thread does get the latest data via a read, but immediately after that, another thread modifies it, does the data of the first thread still up-to-date?

Compared to other memory orders, the pros of `sequential consistent` is that "all threads see the same order", but this also means that "all sequential consistent operations are done serially", so it will inevitably impact the performance of the program.

Theoretically, most operations that using `sequential consistent` can switch to use other memory orders through a proper modification of the code. But we should also be aware that the control of memory order is subtle and mistakes can be made easily, so if you're not sure which memory order to use, choose `sequential consistent` - it's not good to be slow, but better than a bug.

## How memory order is implemented

Memory order is achieved by the cooperation of compiler and CPU. Some CPUs are inherently guarantee certain memory orders, such as x86/64:

* A write will never be reordered to before another write, because reorder of `SS` is not allowed;
* A write will never be reordered to before a read, because reorder of `LS` is not allowed.

That is to say, for a write operation, the CPU guarantees that any operation that precedes it will complete before it, which is the semantics of `release`. In this case, the memory order specified in the code is only to prevent the compiler from an aggressive optimaztion.
...
But unfortunately, this is incorrect, because: although we are using atomic operations, we are using two of them, and concurrent calls can be made between the operations, thus the entire function is not atomic. Thereore, we need to change it as below:
...
As we can see, by putting CAS into a loop, we can detect race conditions and make retries when necesary to ensure the end result is correct. But this hasn't show the real power of CAS!

You may alread notice that the core functionality of the function is done by the "calculate the new value" part of the loop, the atomicity of the function is achieved by the reset parts of the loop, and the two parts are completely independent. This means, by replacing the "calculate the new value" part with a new one, the entire function becomes a new atomic operation, the possibilities are endless!

Moreover, we can go even further, by combine several atomic operations implemented by CAS, we can make many lightweight locks for more complex secenarios, for example, the following single-writer-multiple-reader-lock only uses several bytes of memory, this make it a better choice when we need to use a lot of locks.
...
In the process of push and pop operations, although the code ensures that the stack pointer does not change via CAS, there's a bug in the following execution sequence.
...
This section was not originally in the writing plan, but when I wrote the previous section, I suddenly noticed that there's a problem with the final implementation of the stack, and it has been more than ten years since I first saw such a stack, what an embarrassment!

The problem with the above implementation is at line `newHead.n = oldHead.n->next` of function `stack_pop`, it is safe to access `oldHead.n`, but accessing `oldHead.n->next` is not. Because in a multithreading program, the node that `oldHead.n` points to can be poped out and released by other threads. However, triggering this bug not only requires the program to release the node, but also requires the corresponding memory to be inaccessble. The reason that my program has never gone wrong in more than ten years is that I implemented a memory pool with this stack, and the memory of the nodes was never actually freed.

And what's more embarrassing is, not only I myself made this mistake, for example, [this implementation](https://github.com/reactos/reactos/blob/893a3c9d030fd8b078cbd747eeefd3f6ce57e560/sdk/lib/rtl/i386/interlck.S) in [reactos](https://reactos.org/)(an open-source Windows clone), although written in assembly code, has the same problem.
...
Finally, this problem also shows that how many pitfalls we will meet in multithreaded programming: we saw the ABA problem and carefully avoided it, but defeated by another bug immediately. And, once we meet a bug, we will find out how difficult to fix it: we saw the wrong behavior, but haven't any clue about the root cause as it happened minutes or even days ago, and the crime scene has already been totally destroyed, thus we can only guess. We guessed one, try to verify if we are correct, but then sadly discovery that we cannot reproduce the bug, and have to rely on long-term execution to increase the possibility. This is also embarrassing.

Therefore, be careful and be more careful, try best to not make mistakes will always be the best choice to avoid embarrassment in multi-threaded development.
